...
class MembershipApiController extends Controller
{
    ...
    {
        if (($model = Membership::find($id)) !== null) {
            return $model;
        }
    ...
    public function returnResponse($request, $id, $data = [])
    {
        ...
        if ($option) {
            $successData = Membership::with($withArray)->selectRaw($option['valueField'] . ' as value, ' . $option['labelField'] . ' as label')->where($where)->get()->toArray();
            return response()->json($successData, $this->successStatus);
        }
        if ($id) {
            if ($request->result == 'result_array') {
                $model = Membership::with($withArray)->select($field)->where($where)->get();
            } else {
                $model = Membership::with($withArray)->select($field)->where($where)->first();
            }
            ...
            if ($pagination == true) {
                if ($whereLike) {
                    $model = Membership::with($withArray)->select($field)->where(function ($query) use ($whereLike) {
                        if ($whereLike) {
                    ...
                    $model = Membership::with($withArray)->select($field)->where($where)->orderByRaw($orderby)->paginate($limit);
                }
            } else {
                if ($whereLike) {
                    $model = Membership::with($withArray)->select($field)->where(function ($query) use ($whereLike) {
                    ...
                    $model = Membership::with($withArray)->select($field)->where($where)->orderByRaw($orderby)->get();
                }
            }
            if ($model->count()) {
                $successData = $model->toArray();
