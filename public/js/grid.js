...
(function($) {
        ...
        _grids.utils.handleAjaxRequest = function(element, event, options) {
            ...
            element.each(function(i, obj) {
                ...
                var confirmationMessage =
                    obj.data("confirmation-msg") || "Are you sure?";
                ...
                },
                {
                    ...
                    value: function search() {
                        ...
                        if (form.length > 0) {
                            $(document).on(
                                "submit",
                                this.opts.searchForm,
                                function(event) {
                                    $.pjax.submit(
                                        event,
                                        _this4.opts.id,
                                        _this4.opts.pjax.pjaxOptions
                                    );
                                }
                            );
                        }
    ...
    _grids.formUtils = {
        ...
        handleFormSubmission: function handleFormSubmission(formId, modal) {
            ...
            var notification =
                form.data("notification-el") || "modal-notification";
            var _this = this;
            ...
            $.ajax({
                ...
                success: function success(response) {
                    if (response.success) {
                        ...
                        $("#" + notification).html(
                            _this.renderAlert("success", message)
                        );
    ...
    (function($) {
        ...
        $("#bootstrap_modal").on(
            ...
            function(e) {
                ...
                _grids.formUtils.handleFormSubmission(
                    "modal_form",
                    $("#bootstrap_modal")
                );
            }
    ...
    _grids.init = function() {
        ...
        $("#grid_view_checkbox_main").click(function(event) {
            $('input[role="grid-view-checkbox-item"]').prop(
                "checked",
        ...
        $(document).on("pjax:send", function() {
            $("#gridtable-pjax").addClass("kv-grid-loading");
        });
        $(document).on("pjax:complete", function() {
            $("#gridtable-pjax").removeClass("kv-grid-loading");
        });
        // initialize modal js
        // _grids.modal.init({});
        // table links
