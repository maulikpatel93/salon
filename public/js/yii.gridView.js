...
(function($) {
    $.fn.yiiGridView = function(method) {
        if (methods[method]) {
            ...
            $.error(
                "Method " + method + " does not exist in jQuery.yiiGridView"
            );
            return false;
        }
    };
    ...
    var methods = {
        ...
        applyFilter: function() {
            ...
            $.each(
                yii.getQueryParams(settings.filterUrl),
                function(name, value) {
                    if (
                        namesInFilter.indexOf(name) === -1 &&
                        namesInFilter.indexOf(name.replace(/\[\d*\]$/, "")) ===
                        -1
                    ) {
                        if (!$.isArray(value)) {
                            value = [value];
            ...
            var url =
                pos < 0 ?
                settings.filterUrl :
                settings.filterUrl.substring(0, pos);
        ...
        setSelectionColumn: function(options) {
            ...
            initEventHandler(
                ...
                function() {
                    var all =
                        $grid.find(inputs).length ==
                        $grid.find(inputs + ":checked").length;
                    $grid
