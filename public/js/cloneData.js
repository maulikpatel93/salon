...
(function($) {
    $.fn.cloneData = function(options, callback) {
        ...
        var _updateAttributes = function() {
            ...
            ).each(function(parent_index, item) {
                $(this)
                    .attr("data-index", parent_index)
                    .addClass(settings.copyClass);
            });
            ...
            if (name !== undefined) {
                ...
                if (matches && matches.length === 3) {
                    ...
                    if (identifiers.length > 1) {
                        var widgetsOptions = [];
                        $elem
                            .parents("." + settings.mainContainerId)
                            .each(function(i) {
        ...
        var _parseTemplate = function() {
            ...
            $template.find("input, textarea, select").each(function() {
                if ($(this).is(":checkbox") || $(this).is(":radio")) {
                    ...
                    $(this)
                        .parents(".fileinput")
                        .find(".check-file-remove")
                        .hide();
                    $(this)
                        .parents(".fileinput")
                        .find(".check-file-change")
                        .hide();
                    $(this)
                        .parents(".fileinput")
                        .find(".check-file-select")
                        .show();
        ...
        var _initializePlugins = function() {
            ...
            if ($(".clone.select2").length > 0) {
                $(".clone.select2").each(function() {
                    $(this)
                        .select2({ width: "100%" })
            ...
            if (count > settings.minLimit) {
                if (settings.removeConfirm) {
                    if (confirm(settings.removeConfirmMessage)) {
                        $elem
                            .parents("." + settings.cloneContainer)
                            .slideUp(function() {
                                $(this).remove();
                                _updateAttributes();
