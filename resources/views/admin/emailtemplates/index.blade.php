...
@php
    ...
    if (!empty(checkaccess('create', getControllerName()))) {
        $createbtn = '<a href="'.route('admin.emailtemplates.create').'" class="btn btn-primary showModalButton" title="Add '.$title_single.'" data-bs-toggle="modal" data-bs-target="#gridviewModal">Add</a>';
    }
...
if (!empty(checkaccess('view', getControllerName()))) {
    $actionTypes[] = [
                    ...
                    'url' => function ($model) { // Optional
                        return route('admin.emailtemplates.view', ['id' => encode($model->id)]);
                    },
...
if (!empty(checkaccess('update', getControllerName()))) {
    $actionTypes[] = [
                    ...
                    'url' => function ($model) { // Optional
                        return route('admin.emailtemplates.edit', ['id' => encode($model->id)]);
                    },
                    'htmlAttributes' => [ // Optional
...
if (!empty(checkaccess('delete', getControllerName()))) {
    $actionTypes[] = [
                    ...
                    'url' => function ($model) { // Optional
                        return route('admin.emailtemplates.delete', ['id' => encode($model->id)]);
                    },
                    'htmlAttributes' => [ // Optional
...
@section('content')
        ...
        'rowsFormAction' => route('admin.emailtemplates.applystatus'), // Route url to send slected checkbox items for deleting rows, for example.
        ...
        'columnFields' => [
                    ...
                    'value' => function ($model) { // You can set 'value' as a callback function to get a row data value dynamically.
                        ...
                        if (!empty(checkaccess('isactive', getControllerName()))) {
                            return Html::link('javascript:void(0)', $active, [
                                ...
                                'onclick' =>
                                    '$.post({
                                    url: "' .route('admin.emailtemplates.isactive', ['id' => encode($model->id)]) .'",
            ...
            'toolbar' =>  [
                'content' => '<div class="btn-group" role="group">
                                '.$createbtn.'
                                <a href="'.route("admin.emailtemplates.index").'" class="btn btn-secondary" title="Refresh Salon" data-trigger-pjax="1" ><i class="fas fa-redo"></i></a>
                            </div>
                            ',
                'applybtn' => $applyafter,
            ],
