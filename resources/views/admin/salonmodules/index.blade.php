...
if (!empty(checkaccess('create', getControllerName()))) {
    $createbtn = '<a href="' . route('admin.salonmodules.create') . '" class="btn btn-primary showModalButton" title="Add ' . $title_single . '" data-bs-toggle="modal" data-bs-target="#gridviewModal">Add</a>';
}
...
if (!empty(checkaccess('view', getControllerName()))) {
    $actionTypes[] = [
        ...
        'url' => function ($model) {
            // Optional
            return route('admin.salonmodules.view', ['id' => encode($model->id)]);
        },
        'htmlAttributes' => [
            // Optional
...
if (!empty(checkaccess('update', getControllerName()))) {
    $actionTypes[] = [
        ...
        'url' => function ($model) {
            // Optional
            return route('admin.salonmodules.edit', ['id' => encode($model->id)]);
        },
        'htmlAttributes' => [
            // Optional
...
if (!empty(checkaccess('delete', getControllerName()))) {
    $actionTypes[] = [
        ...
        'url' => function ($model) {
            // Optional
            return route('admin.salonmodules.delete', ['id' => encode($model->id)]);
        },
        'htmlAttributes' => [
            // Optional
...
@section('content')
    ...
    $gridData = [
        ...
        'rowsFormAction' => route('admin.salonmodules.applystatus'), // Route url to send slected checkbox items for deleting rows, for example.
                ...
                'value' => function ($model) {
                    ...
                    if (!empty(checkaccess('isactive', getControllerName()))) {
                        return Html::link('javascript:void(0)', $active, [
                            ...
                            'onclick' =>
                                '$.post({
                                url: "' .route('admin.salonmodules.isactive', ['id' => encode($model->id)]) .'",
        ...
        'toolbar' => [
            'content' =>
                '<div class="btn-group" role="group">
                    ' .$createbtn .'
                    <a href="' .route('admin.salonmodules.index') .'" class="btn btn-secondary" title="Refresh Module" data-trigger-pjax="1" ><i class="fas fa-redo"></i></a>
                </div>', 
            'resetbtn' => false,
            'applybtn' => $applyafter,
        ],
